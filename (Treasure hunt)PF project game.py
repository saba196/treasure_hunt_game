"""
⚓  TREASURE HUNT  ⚓  — v5 ENHANCED (FIXED & FULLY COMMENTED)
==============================================================
...
FIXES APPLIED:
  1. Achievements text on menu is no longer hidden behind buttons
     (all menu buttons shifted down by ~30px so text is fully visible)
  2. Removed unused variable 'e' in write_save() except clause
     (was: except (OSError, IOError) as e — fixed to: except (OSError, IOError))
  3. "Replay Level 1" after finishing all 20 levels now performs a FULL
     reset: clears all stars, coins, score, achievements, and locks every
     level except Level 1 — exactly like a brand-new install.
  4. "Continue Voyage" button only appears after the player has actually
     started playing Level 1 (won or lost at least once), not on a
     completely fresh save.

Run:  python treasure_hunt_enhanced.py
"""
...
class Game:
    def __init__(self, root):
        ...
        """
        self.root = root
        root.title("⚓  TREASURE HUNT  ⚓  — Enhanced Edition")
